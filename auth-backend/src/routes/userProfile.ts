...
userProfileRoutes.post("/profile/init", requireAuth, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const profile = await createUserProfile(userId, {
      softwareDevelopmentExperience: softwareDevelopmentExperience || "beginner",
      roboticsHardwareBackground: roboticsHardwareBackground || "none",
      programmingLanguages: programmingLanguages ? JSON.stringify(programmingLanguages) : "[]",
    });
