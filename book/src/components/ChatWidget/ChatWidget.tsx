  ...
  if (!isOpen) {
    return (
      <div className={`${styles.chatWidget} ${styles.bottomRight} ${styles.closed}`}>
        <button className={styles.floatingBtn} onClick={() => setIsOpen(true)} title="Open Chat">💬</button>
      </div>
    );
  }
  ...
  return (
    <div className={`${styles.chatWidget} ${styles.bottomRight} ${styles.open}`}>
      <div className={styles.header}>
        <div className={styles.headerTitle}><span>🤖</span><span>Textbook Assistant</span></div>
        <div className={styles.headerControls}>
          <div style={{ position: 'relative' }}>
            <button className={styles.menuBtn} onClick={() => setShowMenu(!showMenu)}>⋮</button>
            {showMenu && (
              <div className={styles.dropdown}>
                <button className={styles.dropdownItem} onClick={handleClearHistory}>��️ Clear History</button>
                <button className={styles.dropdownItem} onClick={handleNewSession}>↻ New Session</button>
              </div>
            )}
          </div>
          <button className={styles.toggleBtn} onClick={() => setIsOpen(false)}>✕</button>
        </div>
      </div>
      ...
      <div className={styles.body}>
        ...
        <div className={styles.inputArea}>
          ...
          <div className={styles.buttons}>
            <button className={styles.sendBtn} onClick={() => handleSend(inputValue)} disabled={isLoading || !inputValue.trim()}>Send</button>
            {selectedText && <button className={styles.selectionBtn} onClick={() => handleSend(inputValue, true)} disabled={isLoading || !inputValue.trim()}>Ask About Selection</button>}
          </div>
        </div>
      </div>
    </div>
